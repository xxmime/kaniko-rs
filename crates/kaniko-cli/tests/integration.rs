...
fn parse_stages(dockerfile: &str) -> Result<Vec<dockerfile_parser::Stage>, dockerfile_parser::ParseError> {
...
"#;
    ...
    assert_eq!(stages[0].image, "scratch");
    assert!(!stages[0].instructions.is_empty());
}
...
"#;
    ...
    assert!(base_pos < builder_pos);

    let final_pos = order.iter().position(|&x| x == 2).unwrap();
    assert!(builder_pos < final_pos);
}
